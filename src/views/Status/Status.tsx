...
const Status = ({
  ...
  const [currentSupply, setCurrentSupply]: [
    number,
    Dispatch<SetStateAction<number>>
  ] = useState(0);
  ...
  const [totalBurned, setTotalBurned]: [
    number,
    Dispatch<SetStateAction<number>>
  ] = useState(0);
    ...
    <div className={styles.Status}>
      <div className={styles.Status__greeting}>
        ...
        <h2 className={styles.Status__subtitle}>
          Please familiarize yourself with the Vault Swap contract prior to
          interacting with the vault.
        </h2>
      </div>
      <div className={styles.Status__widgets}>
        <Total currentSupply={currentSupply} totalBurned={totalBurned} />
        <Vault totalBurned={totalBurned} currentVault={currentVault} />
